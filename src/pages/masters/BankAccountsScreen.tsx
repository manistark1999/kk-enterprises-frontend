...
export function BankAccountsScreen({ isDarkMode }: BankAccountsScreenProps) {
  ...
  const getAccountTypeColor = (type: string) => {
    const colors: { [key: string]: string } = {
      ...
      'Current': 'bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-400',
      'Cash Credit': 'bg-orange-100 text-orange-700 dark:bg-orange-500/20 dark:text-orange-400',
      'CC/OD': 'bg-orange-100 text-orange-700 dark:bg-orange-500/20 dark:text-orange-400',
      'Fixed Deposit': 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400',
    };
    ...
    <div className="p-6 h-full flex flex-col space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button 
            ...
            className={`p-2.5 rounded-xl border transition-all ${
              isDarkMode ? 'bg-slate-800 border-slate-700 text-slate-400 hover:text-white hover:bg-slate-700' : 'bg-white border-slate-200 text-slate-500 hover:text-slate-900 hover:bg-slate-50'
            }`}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Active Accounts', value: activeAccounts, icon: ShieldCheck, color: 'indigo' },
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <Activity className="w-4 h-4 text-slate-500 opacity-30" />
            </div>
            <div className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{stat.value}</div>
        ...
        <div className="flex flex-col md:flex-row gap-4 flex-1">
          <div className="relative flex-1">
            ...
            <input
              ...
              className={`w-full pl-10 pr-4 py-2.5 rounded-xl border transition-all ${
                isDarkMode 
                  ? 'bg-slate-900/50 border-slate-700 text-white focus:border-primary' 
                  : 'bg-slate-50 border-slate-200 text-slate-900 focus:border-primary focus:bg-white'
              }`}
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              className={`px-3 py-2.5 rounded-xl border bg-transparent ${isDarkMode ? 'border-slate-700 text-slate-300' : 'border-slate-200 text-slate-700'}`}
            >
      ...
      <div className="flex-1 min-h-0">
        {isLoading ? (
          <div className="h-full flex flex-col items-center justify-center space-y-4">
             ...
             <p className={isDarkMode ? 'text-slate-400' : 'text-slate-500'}>Fetching accounts...</p>
          </div>
        ) : filteredAccounts.length === 0 ? (
          <div className={`h-full flex flex-col items-center justify-center space-y-4 rounded-3xl border-2 border-dashed ${
            isDarkMode ? 'border-slate-800 bg-slate-800/10' : 'border-slate-100 bg-slate-50/50'
          }`}>
            <div className="p-4 rounded-full bg-slate-200 dark:bg-slate-800">
              <Building2 className="w-12 h-12 text-slate-400" />
            </div>
            <div className="text-center">
              <h3 className={`text-lg font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>No Accounts Found</h3>
          ...
          <div className={`h-full overflow-hidden rounded-2xl border ${
            isDarkMode ? 'bg-slate-800/20 border-slate-700' : 'bg-white border-slate-200'
          }`}>
            <div className="h-full overflow-auto scrollbar-thin scrollbar-thumb-slate-700">
              <table className="w-full text-left border-collapse">
                <thead className={`sticky top-0 z-10 ${isDarkMode ? 'bg-slate-800' : 'bg-slate-50'}`}>
                  <tr>
                    <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-slate-500">Bank Details</th>
                    <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-slate-500">Account Type</th>
                    <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-slate-500">Balance</th>
                ...
                <tbody className="divide-y divide-slate-700/30">
                  {filteredAccounts.map((account) => (
                    <motion.tr 
                      layout
                      key={account.id}
                      className={`group transition-colors ${isDarkMode ? 'hover:bg-slate-700/30' : 'hover:bg-slate-50'}`}
                    >
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className={`text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-500'} flex items-center gap-2 mt-1`}>
                      ...
                      <td className="px-6 py-5">
                        <div className={`flex items-center gap-2 font-semibold text-xs ${
                          account.status === 'Active' ? 'text-emerald-500' : 'text-slate-400'
                        }`}>
                          <div className={`w-2 h-2 rounded-full ${account.status === 'Active' ? 'bg-emerald-500' : 'bg-slate-400'}`}></div>
                          {account.status}
                        </div>
                      </td>
              ...
              {filteredAccounts.map((account) => (
                <motion.div
                  ...
                  className={`p-6 rounded-3xl border transition-all hover:scale-[1.02] cursor-pointer group ${
                    isDarkMode ? 'bg-slate-800/40 border-slate-700 hover:border-slate-500' : 'bg-white border-slate-200 shadow-sm hover:shadow-xl'
                  }`}
                  onClick={() => handleOpenDrawer(account)}
                >
                  <div className="flex items-start justify-between mb-8">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 bg-gradient-to-br from-primary to-indigo-600 rounded-2xl flex items-center justify-center text-white text-2xl shadow-lg ring-4 ring-white/10">
                        <Building2 className="w-7 h-7" />
                      </div>
                      <div>
                        <h3 className={`font-bold text-lg leading-tight ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{account.accountName}</h3>
                        <p className={`text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>{account.bankName}</p>
                      </div>
                    </div>
                    <div className="relative group/more">
                      <div className={`p-2 rounded-xl border ${isDarkMode ? 'border-slate-700 text-slate-500' : 'border-slate-100 text-slate-400'}`}>
                        <ShieldCheck className="w-5 h-5" />
                      </div>
                    </div>
                  </div>

                  <div className={`p-4 rounded-2xl mb-6 ${isDarkMode ? 'bg-slate-900/50' : 'bg-slate-50'}`}>
                    <div className="flex items-center justify-between mb-2">
                       <span className="text-xs font-bold text-slate-500 tracking-wider">BALANCE</span>
                       <span className={`px-2 py-0.5 rounded-lg text-[10px] font-black uppercase ${getAccountTypeColor(account.accountType)}`}>
                        {account.accountType}
                      </span>
                    </div>
                    <div className={`text-2xl font-mono font-black ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                      {showBalances ? `₹${(account.currentBalance || 0).toLocaleString('en-IN')}` : '••••••••'}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5 overflow-hidden">
                      ...
                      <span className={`text-sm font-mono truncate ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                        {account.accountNumber}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <button className="p-2 rounded-xl bg-slate-100 dark:bg-slate-700/50 text-slate-500 dark:text-slate-400 opacity-0 group-hover:opacity-100 transition-all scale-90 group-hover:scale-100 translate-x-2 group-hover:translate-x-0">
          ...
          <div className="fixed inset-0 z-50 flex justify-end">
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-thin">
                <section>
                  <h3 className={`text-xs font-black uppercase tracking-widest mb-4 flex items-center gap-2 ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
                    <Building2 className="w-4 h-4" />
                    Core Identity
                  </h3>
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label className={`text-sm font-bold ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Account Nickname *</label>
                      <input 
                        type="text"
                        ...
                        className={`w-full px-4 py-3 rounded-xl border font-medium outline-none transition-all ${
                          isDarkMode ? 'bg-slate-800 border-slate-700 text-white focus:border-primary' : 'bg-slate-50 border-slate-200 text-slate-900 focus:border-primary'
                        }`}
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className={`text-sm font-bold ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Bank Name *</label>
                        <input 
                          type="text"
                      ...
                      <div className="space-y-1.5">
                        <label className={`text-sm font-bold ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Account Type</label>
                        <select
                ...
                <section>
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label className={`text-sm font-bold ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Full Account Number *</label>
                      <input 
                        type="text"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className={`text-sm font-bold ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>IFSC Code *</label>
                        <input 
                          type="text"
                      ...
                      <div className="space-y-1.5">
                        <label className={`text-sm font-bold ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Branch Name</label>
                        <input 
                          type="text"
                    ...
                    <div className="space-y-1.5">
                      <label className={`text-sm font-bold ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Account Holder Name *</label>
                      <input 
                        type="text"
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className={`w-full pl-8 pr-4 py-4 rounded-3xl border font-black text-2xl outline-none transition-all ${
                          isDarkMode ? 'bg-slate-800 border-slate-700 text-emerald-400 focus:border-emerald-500' : 'bg-emerald-50 border-emerald-100 text-emerald-700 focus:border-emerald-500'
                        }`}
                      />
                    </div>
                  </section>
                ...
                <section className="pt-4">
                  <div className={`p-4 rounded-3xl border flex items-center justify-between ${
                    isDarkMode ? 'bg-slate-800/20 border-slate-700' : 'bg-slate-50 border-slate-100'
                  }`}>
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-xl ${formData.status === 'Active' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-500/10 text-slate-500'}`}>
                        <ShieldCheck className="w-5 h-5" />
                      </div>
                      <div>
                        <div className={`font-bold text-sm ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>Account Status</div>
                        <div className="text-xs text-slate-500">Enable or disable transactions</div>
                      </div>
                    </div>
                    <button
                      onClick={() => setFormData({...formData, status: formData.status === 'Active' ? 'Inactive' : 'Active'})}
                      className={`relative w-14 h-8 rounded-full transition-colors ${
                        formData.status === 'Active' ? 'bg-emerald-500' : 'bg-slate-600'
                      }`}
                    >
                ...
                <div className="flex gap-4">
                  <button 
                    ...
                    className={`flex-1 py-4 rounded-2xl font-bold transition-all ${
                      isDarkMode ? 'bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700' : 'bg-white border-2 border-slate-100 text-slate-500 hover:bg-slate-50'
                    }`}
                  >
                    Cancel
                  </button>
